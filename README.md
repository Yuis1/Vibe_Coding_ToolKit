...
## 懒人配置法
zcf 全称是 Zero-Config Code Flow，零配置代码流。

它解决的核心问题：把 Claude Code 的配置过程从手动改配置文件，变成了交互式问答，你只需要回答几个问题，它就自动帮你配置好所有东西。不需要你知道配置文件在哪，不需要你了解 JSON 格式，甚至不需要你懂什么是 MCP 服务，全程傻瓜式操作。
